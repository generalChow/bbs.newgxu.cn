...
newgxu.Dialog = new Class({
	...
	initialize: function(title, content, options){
		...
		if (this.options.overlay) {
			...
			this.overlay.setStyles({
				'position': 'absolute', 'top': 0, 'left': 0, 'background-color':'#99ccff', 'width': '100%', 'visibility': 'hidden'
		...
		if (this.options.overlay) {
			this.fx1 = new Fx.Style(this.overlay, 'opacity', {duration:100}).custom(0, 0.3);
		}
	...
	close: function(fn) {
		...
		if (this.options.overlay) {new Fx.Style(this.overlay, 'opacity', {duration:100}).custom(0.3, 0);}
...
newgxu.Dialog.implement(new Chain);

newgxu.HtmlDialog = newgxu.Dialog.extend({
	initialize: function(dialog, w, h) {
		var options = {effect: 'slide', overlay: true, width: w||430, height: h||290};
		this.parent("", $(dialog), options);
	}
});

newgxu.UbbDialog = newgxu.Dialog.extend({
	initialize: function(dialog, w, h) {
		var options = {effect: 'slide', overlay: true, width: w||340, height: h||240};
		this.parent("", $(dialog), options);
	}
});

newgxu.TextDialog = newgxu.Dialog.extend({
	initialize: function(content) {
		var options = {effect: 'slide', overlay: true, width:300, height: 0};
		this.parent("ʾϢ", content, options);
	}
});

newgxu.ErrorDialog = newgxu.Dialog.extend({
	initialize: function(content) {
		var options = {effect: 'slide', delay: 3000, overlay: true, width:380, height: 0};
		var msgImg = $('msgImg');
		var msgText = $('msgText');
		if (msgImg) msgImg.innerHTML = "<img src='/images/ico-erro.gif' />";
		if (msgText) msgText.innerHTML = content;
		var dlg = $('msgContent');
		this.parent("", dlg||content, options);
	}
});

newgxu.SuccessDialog = newgxu.Dialog.extend({
	initialize: function(content) {
		var options = {effect: 'slide', delay: 3000, overlay: true, width:380, height: 0};
		var msgImg = $('msgImg');
		var msgText = $('msgText');
		if (msgImg) msgImg.innerHTML = "<img src='/images/ico-succeed.gif' />";
		if (msgText) msgText.innerHTML = content;
		var dlg = $('msgContent');
		this.parent("", dlg||content, options);
	}
