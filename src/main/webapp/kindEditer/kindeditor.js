...
_extend(KEvent, {
	init : function(el, event) {
		...
		if(self.keyCode == 13&&self.ctrlKey){
				submitContent();
				submited=!submited;
				return false;
		}
...
_extend(KCmd, {
	...
	createlink : function(url, type,name) {
		var self = this, doc = self.doc, range = self.range;
		self.select();
		...
		if (range.collapsed) {
			var html = '<a href="' + _escape(url) + '" data-name="'+_escape(name)+'" data-ke-src="' + _escape(url) + '" ';
			if (type) {
			...
			K('a[href="__kindeditor_temp_url__"]', doc).each(function() {
				K(this).attr('href', url).attr('data-ke-src', url).attr('data-name',name);
				if (type) {
