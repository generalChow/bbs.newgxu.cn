...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			//图片说明
			'<div class="ke-dialog-row">',
			'<label for="keTitle" style="width:360px;">您还可以：' + '<button name="localImage">本地上传</button>' + '</label>',
			'<label for="keTitle" style="width:360px;">' + lang.imgType + '</label>',
			'</div>',
			'</div>',
			'</div>'
		].join('');
		...
		var localBox=K('[name="localImage"]', div);
		localBox.click(function(e) {
			localImage();
		...
		var uploadbutton = K.uploadbutton({
			...
			afterUpload : function(data) {
				if (data.error === 0) {
					...
					url="<img src='" +url +"' border='0' alt='"+title+"' />";
					...
					alert(data.url);//to fix our server
				}
			}
		});
