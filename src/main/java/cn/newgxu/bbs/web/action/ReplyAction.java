...
public class ReplyAction extends AbstractBaseAction {
	...
	public String submit() throws Exception {
		...
		try {
			...
			if ((System.currentTimeMillis() - model.getUser().getRegisterTime().getTime() < 24 * 3600 * 1000)) 
	...
	public String replyFastDo() throws Exception{
		...
		try {
			...
			ServletActionContext.getResponse().getWriter().write("<script>parent.result('1');</script>");
			if (model.getSynchronousTwitter() == 2) {
				synchronousTwitter();
			}
	...
	private void synchronousTwitter() {
		...
		twitter.setUrl("http://bbs.newgxu.cn/topic.yws?topicId=" + model.getTopicId() + "&forumId=" + model.getForumId());
		twitter.setTitle(model.getTopic().getTitle());
		twitter.synchronousTwitter(model.getUser().getUsername(), model.getUser().getPassword(), 2);
